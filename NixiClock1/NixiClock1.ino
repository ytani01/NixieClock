...
void btn_handler() {
  ...
  uint8_t off_timing = nixieArray.get_off_timing(curNixie);

  if ( digitalRead(PIN_SW1) == LOW ) {
    off_timing++;
    nixieArray.set_off_timing(curNixie, off_timing);
    Serial.println("off_timing="
		   + String(nixieArray.get_off_timing(curNixie)));
  }
...
void setup() {
  ...
  for (int i=0; i < DIGIT_N; i++) {
    nixieDispData.digit[i] = i;
  } // for(i)
  nixieDispData.digit[4] = DIGIT_BLANK;
  nixieDispData.digit[5] = DIGIT_FOG;
